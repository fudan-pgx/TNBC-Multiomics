...
OncoScan_CNV_QC$ndSNPQC_Con <- ifelse(OncoScan_CNV_QC$ndSNPQC >= 28,"Pass","Fail")
...
ndSNPQC_histogram <- gghistogram(OncoScan_CNV_QC, x = "ndSNPQC", rug = FALSE, 
                                      ...
                                      palette = mypal[c(8,7)], xlab = "ndSNPQC Score", ylab = "", 
                                      legend.title = "Type") + 
  mytheme + geom_vline(aes(xintercept=28), colour="#990000", linetype="dashed")+
  theme(legend.position = "none"); ndSNPQC_histogram

panel.ghi <- ggarrange(MAPD_histogram,ndWavinessSD_histogram,ndSNPQC_histogram,nrow = 1,labels = c("g","h","i"))+
  theme(plot.margin = unit(c(0.5,0.5,0,0.5), "cm")); panel.ghi
ggsave("./results/charts/panel ghi.pdf",panel.ghi,width=12,height=4,dpi=300)

fig3 <- ggarrange(panel.a,panel.bc,panel.def,panel.ghi,nrow = 4, widths = c(2,2,2,2))+
  theme(plot.margin = unit(c(0.5,0.5,0,0.5), "cm"));fig3
ggsave("./results/charts/fig3 new.pdf",fig3,width=12,height=16,dpi=300)

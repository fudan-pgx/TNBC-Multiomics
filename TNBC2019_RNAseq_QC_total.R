...
expr.mat.fpkm_g_tt <- expr.mat.fpkm_g[,-grep("_PT",colnames(expr.mat.fpkm_g))]
colnames(expr.mat.fpkm_g_tt) <- gsub("FPKM.|_TT","",colnames(expr.mat.fpkm_g_tt))
dim(expr.mat.fpkm_g_tt)
### [1] 56858   360
write.table(expr.mat.fpkm_g_tt,"./RData/data_expression_file.tsv",sep = "\t",
...
expr.mat.fpkm_g_tt_zscore <-  t(scale(t(expr.mat.fpkm_g_tt)))
write.table(expr.mat.fpkm_g_tt_zscore,"./RData/data_expression_zscore_file.tsv",sep = "\t",
            row.names = TRUE,quote = FALSE)
...
gene_fpkm <- readRDS("./Rdata/gene_fpkm_r56858c448_genesymbol_TNBC2019_20220307.rds")
dim(gene_fpkm)
### Calculate log(expr+1)
log_gene_expr <- apply(gene_fpkm,2,function(x){log2(x+0.01)})
...
# [1] 0.2549659 FPKM.FUSCCTNBC475 (TT)
# rm(list=ls())
...
expr.mat.fpkm<-readRDS("./RData/gene_fpkm_r56858c448_genesymbol_TNBC2019_20220307.rds")
...
ggsave("./results/charts/fig2_0325.pdf",fig2,width=12,height=16.5,dpi=300)

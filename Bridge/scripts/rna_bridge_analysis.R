...
filtered_fpkm_new_pipe <- fpkm_new_pipe[apply(fpkm_new_pipe, MARGIN=1, 
                                              FUN=function(x) {(sum(x==0)/length(x))<=0.3}),]
filtered_fpkm_new_pipe <- filtered_fpkm_new_pipe+0.01
...
filtered_fpkm_old_pipe <- fpkm_old_pipe[apply(fpkm_old_pipe, MARGIN=1, 
                                              FUN=function(x) {(sum(x==0)/length(x))<=0.3}),]
filtered_fpkm_old_pipe <- filtered_fpkm_old_pipe+0.01
...
result_new_pipe <- ttest_result_new_pipe[,c("log2FC","p-value")]
result_new_pipe$gene_symbol <- rownames(result_new_pipe)
result_new_pipe <- result_new_pipe[abs(result_new_pipe$log2FC)>1&result_new_pipe$`p-value`<0.05,]

result_old_pipe <- ttest_result_old_pipe[,c("log2FC","p-value")]
result_old_pipe$gene_symbol <- rownames(result_old_pipe)
result_old_pipe <- result_old_pipe[abs(result_old_pipe$log2FC)>1&result_old_pipe$`p-value`<0.05,]

compare_FC <- merge(result_new_pipe,result_old_pipe,by="gene_symbol")
plot(compare_FC$log2FC.x,as.numeric(compare_FC$log2FC.y))  
#### cor=9881583
cor.test(compare_FC$log2FC.x,as.numeric(compare_FC$log2FC.y))
...
filtered_fpkm_old_pipe_TT <- filtered_fpkm_old_pipe_TT-0.01
...
filtered_fpkm_new_pipe_TT <- filtered_fpkm_new_pipe_TT-0.01

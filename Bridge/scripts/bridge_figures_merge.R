...
compare_FC_plot <- ggscatter(compare_FC, x = "new_pipe_FC", y = "old_pipe_FC",
  ...
  stat_cor(method = "pearson",alternative = "two.sided",r.digits = 4,
           p.digits = 4,label.x = -1, label.y = -4)+
  labs(title="Compare of the log2FC of pipelines", x="New", y="Old")+ mytheme+
...
panel.ab <- ggarrange(NULL,panel.b,widths = c(4,3)) +
...
panel.cdefg <- ggarrange(p1,p2,p3,p4,p5,nrow = 1, labels = c("c","d","e","f","g")) +
  theme(plot.margin = unit(c(0.5,0.5,0,0.5), "cm"));panel.cdefg
ggsave("./results/charts/panel cdefg.pdf",panel.cdefg,width=16,height=4,dpi=300)
...
FUSCCTNBC_JI_forplot$variable <- factor(FUSCCTNBC_JI_forplot$variable, 
                                        levels = c("VarScan","TNscope",
                                                   "TNseq","Filter.Mutation",
                                                   "Final.Mutation"))
...
p7 <- ggscatter(compare_genes_forplo, x = "percentile.y", y = "percentile.x",
                add = "reg.line", conf.int = TRUE, color = "Var1",palette = pal_20,
                add.params = list(linetype=2, color = "lightblue", fill = "lightgray"))+
  stat_cor(method = "pearson",alternative = "two.sided",r.digits = 4,
           p.digits = 4,label.x = 0.1, label.y = 0.01)+xlim(0,0.3)+ylim(0,0.3)+
...
ggsave('./results/charts/hotspot_genes_precentile_scatter new.png',p7,width=5,height=4)

panel.i <- ggarrange(p7, nrow = 1, labels = c("i")) +
  theme(plot.margin = unit(c(0.5,0.5,0,0.5), "cm"));panel.i
ggsave("./results/charts/panel i.pdf",panel.i,width=6,height=4,dpi=300)
...
panel.k <- ggarrange(p10,FUSCCTNBC_CNV_JI_plo, nrow = 1, labels = c("k"),widths = c(2,4)) +
...
ggsave("./results/charts/fig4.pdf",fig4,width=16,height=20,dpi=300)
